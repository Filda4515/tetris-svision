...
export class AboutEntity extends AbstractEntity {
  constructor(parentEntity, x, y, width, height, bkColor) {
    ...
    this.panelBkColor = bkColor || ZXColor.white;
    this.clickCounter = 0;
  } // constructor
  ...
  init() {
    ...
    this.addEntity(
      new ButtonEntity(
        ...
        ZXColor.brightWhite,
        false,
        { align: 'center', topMargin: 2, member: 'titleBar', hoverColor: ZXColor.black, clickColor: ZXColor.black },
      ),
    );
    this.addEntity(new AbstractEntity(this, 1, 9, this.width - 2, this.height - 10, false, this.panelBkColor));

    this.aboutText =
      'TETRIS IS A REMAKE OF THE ORIGINAL 1984 PUZZLE GAME BY ALEXEY PAJITNOV.\n\n' +
      'THIS VERSION FAITHFULLY RECREATES THE AUTHENTIC LOOK AND FEEL OF THE CLASSIC 1987 MIRRORSOFT (48K) RELEASE FOR THE SINCLAIR ZX SPECTRUM.\n\n' +
      'ENGINE: SVISION\nAUTHOR: FILIP NEVŘALA';

    this.addEntity(
      new TextEntity(this, this.app.fonts.fonts5x5, 6, 14, this.width - 12, this.height - 32, this.aboutText, ZXColor.black, false, {
        align: 'center',
        textWrap: true,
        margin: 2,
        member: 'aboutText',
    ...
    this.addEntity(
      new ButtonEntity(
        this,
        this.app.fonts.fonts5x5,
        Math.floor((this.width - 36) / 2),
        this.height - 16,
        36,
        13,
        'CLOSE',
        { id: 'closeAbout' },
        ['Escape', 'GamepadExit', 'Enter', 'GamepadOK'],
        ZXColor.brightWhite,
        ZXColor.blue,
        { align: 'center', margin: 4 },
      ),
    );
  ...
  updateAbout() {
    switch (Math.floor(this.clickCounter / 5)) {
      ...
      case 2:
        ...
        if (window.clientIP && window.clientIP.indexOf(':') >= 0) {
          ipLabelSeparator = '\n';
        }
        var sysInfoText =
          'version: ' +
          ...
          '\n' +
          'element: ' +
          this.app.element.clientWidth +
          ' x ' +
          this.app.element.clientHeight +
          '\n' +
          'canvas: ' +
          this.app.element.width +
          ' x ' +
          this.app.element.height +
          '\n' +
          'ratio: ' +
          this.app.layout.ratio +
          '\n' +
          'ip:' +
          ipLabelSeparator +
